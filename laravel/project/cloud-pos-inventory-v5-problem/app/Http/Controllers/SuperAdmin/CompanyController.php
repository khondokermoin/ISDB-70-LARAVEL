...
class CompanyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Stats are always computed from ALL companies (not filtered by search)
        $stats = [
            'total'     => Company::count(),
    ...
    public function store(\App\Http\Requests\SuperAdmin\StoreCompanyRequest $request)
    {
        ...
        DB::beginTransaction();
        try {
            // ✅ Use createQuietly() to suppress the 'created' observer event DURING the
            // transaction. The CompanyObserver fires TenantProvisioningService::provision()
            // which runs INSERT queries. If those fail, they would roll back the company too.
            // We call provision() manually AFTER DB::commit() to keep it outside the transaction.
            $company = Company::withoutEvents(function () use ($validated, $request, $themeSettings, $socialLinks, $contactInfo, $trialEndsAt) {
            ...
            // ✅ Fire provisioning AFTER the transaction is committed so that:
            // 1. The company record is guaranteed to exist in the DB.
            // 2. Any provisioning failure does NOT roll back the company creation.
            // 3. The observer's created() event is dispatched here safely.
            $company->fireModelEvent('created', false);
            ...
            return back()->withInput()->with('error', 'Something went wrong while creating the company: ' . $e->getMessage());
        }
    }
